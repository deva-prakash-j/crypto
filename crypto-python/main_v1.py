...
BASE_FEATURE_COLUMNS = [
    ...
    'return_lag1', 'rsi_lag1', 'volume_lag1',
    'volatility_zscore', 'volume_zscore',
    'ema_50', 'high_100', 'low_100'
]
...
def add_features(df):
    ...
    df['trend_slope'] = df['close'].rolling(window=10).apply(lambda x: LinearRegression().fit(np.arange(len(x)).reshape(-1,1), x).coef_[0] if len(x)==10 else np.nan)
    ...
    df['volatility_zscore'] = (df['volatility'] - df['volatility'].rolling(20).mean()) / df['volatility'].rolling(20).std()
    df['volume_zscore'] = (df['volume'] - df['volume'].rolling(20).mean()) / df['volume'].rolling(20).std()

    # Long-term indicators
    df['ema_50'] = EMAIndicator(df['close'], window=50).ema_indicator()
    df['high_100'] = df['high'].rolling(window=100).max()
    df['low_100'] = df['low'].rolling(window=100).min()
...
if __name__ == '__main__':
    ...
    all_features = BASE_FEATURE_COLUMNS + symbol_features

    model = RandomForestClassifier(n_estimators=100, random_state=42)
    model.fit(full_df[all_features], full_df['target'])

    print("\n\U0001F680 Backtest with enriched features:")
    results = backtest_signals(full_df, model, all_features)
